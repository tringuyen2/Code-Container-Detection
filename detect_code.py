...
# parser.add_argument('--show_time', default=True,
#                     action='store_true', help='show processing time')
# parser.add_argument('--test_folder', default='image-test',
...
def test_net(net, image, text_threshold, link_threshold, low_text, cuda, poly, refine_net=None):
    ...
    score_link = imgproc.cvt2HeatmapImg(score_link)



    if show_time:
        print("\ninfer/postproc time (CRAFT) : {:.3f}/{:.3f}".format(t0, t1))
...
def detect_code(image_path='img_crop4.png', root_image='imc.png', position_crop=(0,0)):
    # load net
    ...
    if cuda:
        ...
        net.load_state_dict(copyStateDict(torch.load(
            trained_model, map_location='cpu')))
    ...
    image, r = imgproc.loadImage(image_path)


    # from skimage import io
    # base_image = io.imread(root_image)
    # if base_image.shape[0] == 2: base_image = base_image[0]
    # if len(base_image.shape) == 2 : base_image = cv2.cvtColor(base_image, cv2.COLOR_GRAY2RGB)
    # if base_image.shape[2] == 4:   base_image = base_image[:,:,:3]
    base_image = cv2.imread(root_image)
    ...
    file_utils.saveResult(
        image_path, image[:, :, ::-1], base_image, polys, r, dirname=result_folder, position_crop=(p1,p2))

    

    print("Time CRAFT: {}s".format(time.time() - t))
